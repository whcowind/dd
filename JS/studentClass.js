	...
	$('.class').find('.student-data').live("click" , function(){
		...
		$('.class').find('.student-detail').css('display' , 'inline-block');
			...
			success:function(response){
				...
				for (var i = 0; i < response.length; i++) {
					if (response[i].paper == "") {
						...
						tcount -= 1;
						normalEnd = tcount - response[i]['paper']['title'].gpcount;
						gpStart = normalEnd;

						console.log(tcount + ' . ' + normalEnd + ' . ' + gpStart);

						for (var j = 0; j < normalEnd; j++) {
							$('.class .studentexceldetailed .studetailed .' + aid).append("<td>" + response[i]['paper']['title'][j].title + "</td>");
						}

						for (var k = gpStart; k < tcount; k++) {
							var subcount = response[i]['paper']['title'][k].subcount;
							for (var x = 1; x <= subcount; x++) {
								$('.class .studentexceldetailed .studetailed .' + aid).append("<td>" + response[i]['paper']['title'][k]['sub'][x].content + "</td>");
							}
						}
	...
	$('.gradeMainContent').find('.paper-data').live("click" , function(){
		...
		$.ajax({
			...
			success:function(response){
				...
				titleidx -= 1;
				tcount = tcount - 1 - response['title'].gpcount;
				var gpidx = tcount;
